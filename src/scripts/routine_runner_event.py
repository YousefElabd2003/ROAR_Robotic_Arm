...
class RoutineRunnerEvent(Node):
    def __init__(self, motions: list[str], distance_cm: float):
        super().__init__("routine_runner_event")
    ...
    def start(self):
        ...
        if not self._resolve_transform_pair():
            ...
            return
        self.get_logger().info(
            f"Starting event-based routine: steps={len(self.motions)} distance={self.distance_cm:.2f} cm"
        )
...
def main():
    ...
    rclpy.init()
    node = RoutineRunnerEvent(motions=motions, distance_cm=distance_cm)
    node.start()
